              ...
              <button key={product.id} onClick={() => setSelected(product)} className="group w-full text-left" aria-label={`Ver fotos de ${product.name}`}>
                <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#e8e4dd] sm:aspect-[3/4] md:aspect-[4/5]">
                  <Image
                    ...
                    className={`object-contain object-center transition-transform duration-700 ease-out group-hover:scale-[1.035] ${
                      product.id.startsWith("shoulder-bag-piauiense-serra-da-capivara") ? "md:object-contain" : "md:object-cover"
                    }`}
                    priority={index < 3}
                  />
